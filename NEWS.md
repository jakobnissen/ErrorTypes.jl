...
* New function: `unwrap_err`. Unwraps the wrapped error type of a `Result`, and throws an error if the `Result` conains an `Ok`
* New function: `expect_err`. Similar to `unwrap_err`, but throws an error with a custom message.
